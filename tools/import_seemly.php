...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/seemly_categorized.csv';
...
function ensureSeemlyCategory(mysqli $m, int $language_id, int $clothing_id): int {
    $cols = ensureCategoryColumns($m);
    $existing = getCategoryByNameAndParent($m, $language_id, 'Seemly', $clothing_id);
    if ($existing) {
    ...
    return createChildCategory($m, $language_id, 'Seemly', $clothing_id, $cols);
}
...
function parseCategoryChain(string $raw): array {
    ...
    // Supports forms like: "Tops", "Tops > Graphic", "Tops/Graphic"
...
function downloadImage(string $url, string $sku, string $root): string {
    ...
    return file_exists($full) ? ('catalog/seemly/' . $filename) : '';
}

$categoryColumns = ensureCategoryColumns($m);
$seemly_id = ensureSeemlyCategory($m, $language_id, $clothing_id);
...
while (($row = fgetcsv($h)) !== false) {
    ...
    // SKU column is typically VARCHAR(64). Normalize to avoid truncation collisions.
    ...
    if ($existing) {
        ...
        $ps = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_store WHERE product_id=$product_id AND store_id=0");
        if (!$ps || !$ps->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_store (product_id, store_id) VALUES ($product_id, 0)");
        }

        $pcBrand = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$seemly_id");
        if (!$pcBrand || !$pcBrand->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $seemly_id)");
        }

        if ($targetCategoryId !== $seemly_id) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $targetCategoryId)");
        }

        $pcParent = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$clothing_id");
        if (!$pcParent || !$pcParent->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $clothing_id)");
        }

        // Keep only one direct Seemly child/subcategory assignment per product.
        $childQuery = $m->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id=$seemly_id");
        $childIds = [];
        if ($childQuery) {
...
$verify = $m->query("SELECT COUNT(DISTINCT product_id) AS c FROM " . DB_PREFIX . "product_to_category WHERE category_id=$seemly_id");
$count = ($verify && ($r = $verify->fetch_assoc())) ? (int)$r['c'] : 0;

$m->close();

echo "Inserted=$inserted Updated=$updated Skipped=$skipped SeemlyCategoryID=$seemly_id SeemlyProducts=$count" . PHP_EOL;
