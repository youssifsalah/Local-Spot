...
$idSql = "SELECT c.category_id
          ...
          WHERE c.parent_id=492 AND LOWER(cd.name) LIKE 'blackedition%'
          ORDER BY c.category_id DESC
          LIMIT 1";
$idRes = $m->query($idSql);
$catId = ($idRes && ($idRow = $idRes->fetch_assoc())) ? (int)$idRow['category_id'] : 0;

$direct = 0;
$tree = 0;

if ($catId > 0) {
    $q1 = $m->query("SELECT COUNT(DISTINCT product_id) c FROM " . DB_PREFIX . "product_to_category WHERE category_id=$catId");
    ...
    $q2 = $m->query("SELECT COUNT(DISTINCT p2c.product_id) c
                     ...
                     WHERE cp.path_id=$catId");
    if ($q2 && ($r2 = $q2->fetch_assoc())) {
        $tree = (int)$r2['c'];
    }
}

echo 'BlackeditionCategoryID=' . $catId . PHP_EOL;
echo 'BlackeditionDirectProducts=' . $direct . PHP_EOL;
echo 'BlackeditionTreeProducts=' . $tree . PHP_EOL;
