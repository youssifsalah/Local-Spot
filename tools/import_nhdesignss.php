...
$statePath = $root . '/tools/.nhdesignss_state.json';
$start = null;
$limit = 200;
$downloadImages = true;
$brandImagePath = '';
...
foreach ($argv as $i => $arg) {
    if ($arg === '--start' && isset($argv[$i + 1])) {
        $start = max(0, (int)$argv[$i + 1]);
    } elseif ($arg === '--limit' && isset($argv[$i + 1])) {
        $limit = max(1, (int)$argv[$i + 1]);
    } elseif ($arg === '--no-images') {
        $downloadImages = false;
    } elseif ($arg === '--state' && isset($argv[$i + 1])) {
        $statePath = (string)$argv[$i + 1];
    } elseif ($arg === '--brand-image' && isset($argv[$i + 1])) {
        $brandImagePath = (string)$argv[$i + 1];
    } elseif ($arg === '--zero-price' && isset($argv[$i + 1])) {
        $zeroPrice = (float)$argv[$i + 1];
    }
}

if ($start === null && file_exists($statePath)) {
    $stateJson = file_get_contents($statePath);
    $state = json_decode($stateJson, true);
    if (is_array($state) && isset($state['next_index'])) {
        $start = max(0, (int)$state['next_index']);
    }
}
if ($start === null) {
    $start = 0;
...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/nhdesignss_fashion.csv';
...
function ensureBrandCategory(mysqli $m, int $language_id, int $clothing_id, string $brandName): int {
    global $brandImagePath;
    $cols = ensureCategoryColumns($m);
    $existing = getCategoryByNameAndParent($m, $language_id, $brandName, $clothing_id);

    if ($existing) {
        $m->query("UPDATE " . DB_PREFIX . "category SET status=1, parent_id=$clothing_id WHERE category_id=$existing");
        if ($brandImagePath !== '') {
            $imgEsc = $m->real_escape_string($brandImagePath);
            $m->query("UPDATE " . DB_PREFIX . "category SET image='$imgEsc' WHERE category_id=$existing");
        }
    ...
    $newId = createChildCategory($m, $language_id, $brandName, $clothing_id, $cols);
    if ($brandImagePath !== '') {
        $imgEsc = $m->real_escape_string($brandImagePath);
        $m->query("UPDATE " . DB_PREFIX . "category SET image='$imgEsc' WHERE category_id=$newId");
    }
    return $newId;
}
...
function downloadImage(string $url, string $sku, string $root): string {
    global $downloadImages;
    if (!$downloadImages) return '';
    if (!$url) return '';

    $dir = $root . '/image/catalog/nhdesignss';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
    ...
    return file_exists($full) ? ('catalog/nhdesignss/' . $filename) : '';
}

$brandName = 'NH Designs';
$categoryColumns = ensureCategoryColumns($m);
$brand_id = ensureBrandCategory($m, $language_id, $clothing_id, $brandName);
...
while (($row = fgetcsv($h)) !== false) {
    ...
    $targetCategoryId = $brand_id;

    if ($categoryChain) {
        $targetCategoryId = getOrCreateNestedCategory(
            $m,
            $language_id,
            $brand_id,
            $categoryColumns,
    ...
    if ($existingRow) {
        ...
        $imageSet = ($imagePath !== '') ? "image='$imageEsc'," : '';
        ...
        $ps = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_store WHERE product_id=$product_id AND store_id=0 LIMIT 1");
        if (!$ps || !$ps->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_store (product_id, store_id) VALUES ($product_id, 0)");
        }

        $pcBrand = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$brand_id");
        if (!$pcBrand || !$pcBrand->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $brand_id)");
        }

        if ($targetCategoryId !== $brand_id) {
            $pcTarget = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$targetCategoryId");
            if (!$pcTarget || !$pcTarget->num_rows) {
                $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $targetCategoryId)");
            }
        }

        $childIds = [];
        $childQuery = $m->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id=$brand_id");
        if ($childQuery) {
            while ($child = $childQuery->fetch_assoc()) {
                $childIds[] = (int)$child['category_id'];
            }
        }

        if ($childIds) {
            $in = implode(',', $childIds);
            $m->query("DELETE FROM " . DB_PREFIX . "product_to_category
                       WHERE product_id=$product_id AND category_id IN ($in) AND category_id <> $targetCategoryId");
        }
...
$eof = feof($h);
fclose($h);
$nextIndex = $start + $processed;
$done = $eof;
$stateOut = [
    'next_index' => $done ? 0 : $nextIndex,
    'done' => $done,
    'last_run' => date('c'),
];
file_put_contents($statePath, json_encode($stateOut, JSON_PRETTY_PRINT));

$verify = $m->query("SELECT COUNT(DISTINCT product_id) AS c FROM " . DB_PREFIX . "product_to_category WHERE category_id=$brand_id");
$count = 0;
if ($verify && ($vr = $verify->fetch_assoc())) {
    $count = (int)$vr['c'];
}

$m->close();

echo "Inserted=$inserted Updated=$updated Skipped=$skipped Processed=$processed Start=$start Limit=$limit Done=" . ($done ? 'yes' : 'no') . " NextIndex=" . ($done ? 0 : $nextIndex) . " NHDesignsCategoryID=$brand_id NHDesignsProducts=$count" . PHP_EOL;
