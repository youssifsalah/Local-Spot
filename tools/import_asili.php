...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/asili_fashion.csv';
...
function ensureAsiliCategory(mysqli $m, int $language_id, int $clothing_id): int {
    $cols = ensureCategoryColumns($m);
    $existing = getCategoryByNameAndParent($m, $language_id, 'Asili', $clothing_id);
    ...
    return createChildCategory($m, $language_id, 'Asili', $clothing_id, $cols);
}
...
function downloadImage(string $url, string $sku, string $root): string {
    ...
    return file_exists($full) ? ('catalog/asili/' . $filename) : '';
}

$categoryColumns = ensureCategoryColumns($m);
$asili_id = ensureAsiliCategory($m, $language_id, $clothing_id);
...
while (($row = fgetcsv($h)) !== false) {
    ...
    if ($existingRow) {
        ...
        $pcBrand = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$asili_id");
        if (!$pcBrand || !$pcBrand->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $asili_id)");
        }

        if ($targetCategoryId !== $asili_id) {
        ...
        $childIds = [];
        $childQuery = $m->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id=$asili_id");
        if ($childQuery) {
...
$verify = $m->query("SELECT COUNT(DISTINCT product_id) AS c FROM " . DB_PREFIX . "product_to_category WHERE category_id=$asili_id");
$count = 0;
...
echo "Inserted=$inserted Updated=$updated Skipped=$skipped AsiliCategoryID=$asili_id AsiliProducts=$count" . PHP_EOL;
