...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/blackedition_fashion.csv';
...
function ensureBrandCategory(mysqli $m, int $language_id, int $clothing_id, string $brandName): int {
    ...
    return createChildCategory($m, $language_id, $brandName, $clothing_id, $cols);
}
...
function downloadImage(string $url, string $sku, string $root): string {
    ...
    $dir = $root . '/image/catalog/blackedition';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
    ...
    return file_exists($full) ? ('catalog/blackedition/' . $filename) : '';
}

$brandName = 'Blackedition';
...
echo "Inserted=$inserted Updated=$updated Skipped=$skipped BlackeditionCategoryID=$brand_id BlackeditionProducts=$count" . PHP_EOL;
