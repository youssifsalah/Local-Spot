...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/corebasics_fashion.csv';
...
function downloadImage(string $url, string $sku, string $root): string {
    ...
    $dir = $root . '/image/catalog/corebasics';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
    ...
    return file_exists($full) ? ('catalog/corebasics/' . $filename) : '';
}

$brandName = 'Corebasics';
...
echo "Inserted=$inserted Updated=$updated Skipped=$skipped CorebasicsCategoryID=$brand_id CorebasicsProducts=$count" . PHP_EOL;
