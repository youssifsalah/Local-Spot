...
$csvPath = 'c:/Users/Youssif/Desktop/starnine/starnine_scraper/data/antikka_fashion.csv';
...
function ensureAntikkaCategory(mysqli $m, int $language_id, int $clothing_id): int {
    $cols = ensureCategoryColumns($m);
    $existing = getCategoryByNameAndParent($m, $language_id, 'Antikka', $clothing_id);
    ...
    return createChildCategory($m, $language_id, 'Antikka', $clothing_id, $cols);
}
...
function downloadImage(string $url, string $sku, string $root): string {
    ...
    $dir = $root . '/image/catalog/antikka';
    if (!is_dir($dir)) {
        mkdir($dir, 0777, true);
    }
    ...
    return file_exists($full) ? ('catalog/antikka/' . $filename) : '';
}

$categoryColumns = ensureCategoryColumns($m);
$antikka_id = ensureAntikkaCategory($m, $language_id, $clothing_id);
...
while (($row = fgetcsv($h)) !== false) {
    ...
    if ($existingRow) {
        ...
        $pcBrand = $m->query("SELECT 1 FROM " . DB_PREFIX . "product_to_category WHERE product_id=$product_id AND category_id=$antikka_id");
        if (!$pcBrand || !$pcBrand->num_rows) {
            $m->query("INSERT IGNORE INTO " . DB_PREFIX . "product_to_category (product_id, category_id) VALUES ($product_id, $antikka_id)");
        }

        if ($targetCategoryId !== $antikka_id) {
        ...
        $childIds = [];
        $childQuery = $m->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE parent_id=$antikka_id");
        if ($childQuery) {
...
$verify = $m->query("SELECT COUNT(DISTINCT product_id) AS c FROM " . DB_PREFIX . "product_to_category WHERE category_id=$antikka_id");
$count = 0;
...
echo "Inserted=$inserted Updated=$updated Skipped=$skipped AntikkaCategoryID=$antikka_id AntikkaProducts=$count" . PHP_EOL;
