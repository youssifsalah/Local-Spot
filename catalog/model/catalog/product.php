...
class Product extends \Opencart\System\Engine\Model {
	...
	public function getProducts(array $data = []): array {
		...
		$product_data = $this->deduplicateProducts($product_data);

		if ($has_pagination) {
			$product_data = array_slice($product_data, $start, $limit);
		}
	...
	/**
	 * Keep one product per (brand + name), preferring complete records.
	 *
	...
	private function deduplicateProducts(array $products): array {
		...
		foreach ($products as $product) {
			...
			$key = $name !== '' ? (oc_strtolower($name) . '|' . $manufacturer_id) : ('pid|' . $product_id);

			$description = trim(strip_tags(html_entity_decode((string)($product['description'] ?? ''), ENT_QUOTES, 'UTF-8')));
			$description_score = $description !== '' ? oc_strlen($description) : 0;
			$image_score = !empty($product['image']) ? 1 : 0;
			$completeness_boost = $description_score > 0 ? 100000 : 0;
			$score = $completeness_boost + $description_score + $image_score;
			...
			if ($score > $best_score_by_key[$key]) {
				$best_by_key[$key] = $product;
				$best_score_by_key[$key] = $score;
			}
		}
