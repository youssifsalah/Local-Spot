...
class Login extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if ($this->customer->isLogged()) {
			...
			$redirect = $this->customer->getGroupId() === $this->getSellerGroupId()
				? $this->url->link('account/seller', 'language=' . $this->config->get('config_language') . '&customer_token=' . $this->session->data['customer_token'], true)
				: $this->url->link('common/home', 'language=' . $this->config->get('config_language'), true);
		...
		$data['seller_redirect'] = $this->url->link('account/seller', 'language=' . $this->config->get('config_language'));
		$data['buyer_redirect'] = $this->url->link('account/account', 'language=' . $this->config->get('config_language'));
	...
	public function login(): void {
		...
		if (!$json) {
			...
			$is_seller = $customer_info && ((int)$customer_info['customer_group_id'] === $this->getSellerGroupId());

			// Added strpos check to pass McAfee PCI compliance test (http://forum.opencart.com/viewtopic.php?f=10&t=12043&p=151494#p151295)
			if ($is_seller) {
				$json['redirect'] = $this->url->link('account/seller', 'language=' . $this->config->get('config_language') . '&customer_token=' . $this->session->data['customer_token'], true);
			} elseif ($redirect && str_starts_with($redirect, $this->config->get('config_url'))) {
				$json['redirect'] = $redirect . '&customer_token=' . $this->session->data['customer_token'];
			} else {
				$json['redirect'] = $this->url->link('common/home', 'language=' . $this->config->get('config_language'), true);
			}
		}
