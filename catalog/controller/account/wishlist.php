...
class WishList extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->session->data['success'])) {
			...
			$data['button_clear_wishlist'] = $this->language->get('button_clear_wishlist');
			...
			$data['clear'] = $this->url->link('account/wishlist.clear', 'language=' . $this->config->get('config_language') . (isset($this->session->data['customer_token']) ? '&customer_token=' . $this->session->data['customer_token'] : ''));
	...
	protected function getList(): string {
		...
		// Generate larger thumbs for card layout to avoid pixelation when CSS displays 250px image boxes.
		$wishlist_width  = (int)$this->config->get('config_image_wishlist_width');
		$wishlist_height = (int)$this->config->get('config_image_wishlist_height');
		$product_width   = (int)$this->config->get('config_image_product_width');
		$product_height  = (int)$this->config->get('config_image_product_height');
		$thumb_width     = max($wishlist_width, $product_width, 500);
		$thumb_height    = max($wishlist_height, $product_height, 500);
	...
	public function remove(): void {
		...
		if (!empty($this->session->data['wishlist']) && is_array($this->session->data['wishlist'])) {
			$key = array_search($product_id, array_map('intval', $this->session->data['wishlist']), true);
	...
	public function clear(): void {
		...
		if (!$this->customer->isLogged()) {
			$json['error'] = sprintf($this->language->get('error_login'), $this->url->link('account/login', 'language=' . $this->config->get('config_language')), $this->url->link('account/register', 'language=' . $this->config->get('config_language')), $this->url->link('account/wishlist', 'language=' . $this->config->get('config_language')));
		}

		if (!$json) {
			$this->load->model('account/wishlist');

			$this->model_account_wishlist->deleteWishlist($this->customer->getId());
