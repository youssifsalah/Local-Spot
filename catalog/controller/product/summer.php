...
/**
 * Summer Collection page with keyword filters.
 */
class Summer extends \Opencart\System\Engine\Controller {
	/**
	 * Index
	 *
	 * @return void
	...
	public function index(): void {
		...
		if (!empty($this->request->get['summer_tags'])) {
			foreach (explode(',', (string)$this->request->get['summer_tags']) as $tag) {
		...
		if (!empty($this->request->get['summer_brands'])) {
			foreach (explode(',', (string)$this->request->get['summer_brands']) as $brand_id) {
				$id = (int)$brand_id;
		...
		$base_keywords[] = 'summer';
		$exclude_keywords = ['hoodie', 'hoodies', 'zipper', 'jacket', 'crewneck', 'sweatshirt', 'sweater', 'coat', 'winter'];
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('product/summer', 'language=' . $this->config->get('config_language'))
		];

		$this->document->setTitle("Summer Collection");
		...
		if (!empty($selected_tags)) {
			$url .= '&summer_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$url .= '&summer_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['heading_title'] = "Summer Collection";
		$data['text_compare'] = sprintf($this->language->get('text_compare'), isset($this->session->data['compare']) ? count($this->session->data['compare']) : 0);
		$data['compare'] = $this->url->link('product/compare', 'language=' . $this->config->get('config_language'));
		$data['selected_summer_tags'] = $selected_tags;
		$data['summer_filters'] = $filter_options;
		$data['selected_summer_brands'] = $selected_brand_ids;
		$data['summer_brand_filters'] = $brand_filter_options;

		$sort_url = '';

		if (!empty($selected_tags)) {
			$sort_url .= '&summer_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$sort_url .= '&summer_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['sorts'][] = [
			...
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=p.sort_order&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_asc'),
			'value' => 'pd.name-ASC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'pd.name-DESC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=DESC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'p.price-ASC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'p.price-DESC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=DESC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_model_asc'),
			'value' => 'p.model-ASC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=p.model&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_model_desc'),
			'value' => 'p.model-DESC',
			'href'  => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . '&sort=p.model&order=DESC' . $sort_url)
		];
		...
		if (!empty($selected_tags)) {
			$pagination_url .= '&summer_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$pagination_url .= '&summer_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('product/summer', 'language=' . $this->config->get('config_language') . $pagination_url . '&page={page}')
		]);
		...
		$this->response->setOutput($this->load->view('product/summer', $data));
	}
