...
/**
 * Men's Collection page with keyword and brand filters.
 */
class Men extends \Opencart\System\Engine\Controller {
	/**
	 * Index
	 *
	 * @return void
	...
	public function index(): void {
		...
		if (!empty($this->request->get['men_tags'])) {
			foreach (explode(',', (string)$this->request->get['men_tags']) as $tag) {
		...
		if (!empty($this->request->get['men_brands'])) {
			foreach (explode(',', (string)$this->request->get['men_brands']) as $brand_id) {
				$id = (int)$brand_id;
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('product/men', 'language=' . $this->config->get('config_language'))
		];

		$this->document->setTitle("Men's Collection");
		...
		if (!empty($selected_tags)) {
			$url .= '&men_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$url .= '&men_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['heading_title'] = "Men's Collection";
		$data['text_compare'] = sprintf($this->language->get('text_compare'), isset($this->session->data['compare']) ? count($this->session->data['compare']) : 0);
		$data['compare'] = $this->url->link('product/compare', 'language=' . $this->config->get('config_language'));
		$data['selected_men_tags'] = $selected_tags;
		$data['men_filters'] = $filter_options;
		$data['selected_men_brands'] = $selected_brand_ids;
		$data['men_brand_filters'] = $brand_filter_options;

		$sort_url = '';

		if (!empty($selected_tags)) {
			$sort_url .= '&men_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$sort_url .= '&men_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['sorts'][] = [
			...
			'href'  => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . '&sort=p.sort_order&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_asc'),
			'value' => 'pd.name-ASC',
			'href'  => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'pd.name-DESC',
			'href'  => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=DESC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'p.price-ASC',
			'href'  => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'p.price-DESC',
			'href'  => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=DESC' . $sort_url)
		];
		...
		if (!empty($selected_tags)) {
			$pagination_url .= '&men_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$pagination_url .= '&men_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('product/men', 'language=' . $this->config->get('config_language') . $pagination_url . '&page={page}')
		]);
		...
		$this->response->setOutput($this->load->view('product/men', $data));
	}
