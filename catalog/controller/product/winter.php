...
/**
 * Winter Collection page with keyword filters.
 */
class Winter extends \Opencart\System\Engine\Controller {
	/**
	 * Index
	 *
	 * @return void
	...
	public function index(): void {
		...
		if (!empty($this->request->get['winter_tags'])) {
			foreach (explode(',', (string)$this->request->get['winter_tags']) as $tag) {
		...
		if (!empty($this->request->get['winter_brands'])) {
			foreach (explode(',', (string)$this->request->get['winter_brands']) as $brand_id) {
				$id = (int)$brand_id;
		...
		$base_keywords = array_keys($filter_options);
		$base_keywords[] = 'sweatshirt';
		$base_keywords[] = 'winter';
		$exclude_keywords = ['tee', 'tees', 't-shirt', 'tshirt', 'top', 'tops', 'tank', 'short', 'shorts', 'shirt'];
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('product/winter', 'language=' . $this->config->get('config_language'))
		];

		$this->document->setTitle("Winter Collection");
		...
		foreach ($all_results as $product) {
			if (!$this->matchesKeywords($product, $base_keywords)) {
				continue;
			}

			if ($this->matchesKeywords($product, $exclude_keywords)) {
				continue;
			}
		...
		if (!empty($selected_tags)) {
			$url .= '&winter_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$url .= '&winter_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['heading_title'] = "Winter Collection";
		$data['text_compare'] = sprintf($this->language->get('text_compare'), isset($this->session->data['compare']) ? count($this->session->data['compare']) : 0);
		$data['compare'] = $this->url->link('product/compare', 'language=' . $this->config->get('config_language'));
		$data['selected_winter_tags'] = $selected_tags;
		$data['winter_filters'] = $filter_options;
		$data['selected_winter_brands'] = $selected_brand_ids;
		$data['winter_brand_filters'] = $brand_filter_options;

		$sort_url = '';

		if (!empty($selected_tags)) {
			$sort_url .= '&winter_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$sort_url .= '&winter_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['sorts'][] = [
			...
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=p.sort_order&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_asc'),
			'value' => 'pd.name-ASC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'pd.name-DESC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=pd.name&order=DESC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'p.price-ASC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'p.price-DESC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=p.price&order=DESC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_model_asc'),
			'value' => 'p.model-ASC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=p.model&order=ASC' . $sort_url)
		];
		$data['sorts'][] = [
			'text'  => $this->language->get('text_model_desc'),
			'value' => 'p.model-DESC',
			'href'  => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . '&sort=p.model&order=DESC' . $sort_url)
		];
		...
		if (!empty($selected_tags)) {
			$pagination_url .= '&winter_tags=' . implode(',', $selected_tags);
		}
		if (!empty($selected_brand_ids)) {
			$pagination_url .= '&winter_brands=' . implode(',', $selected_brand_ids);
		}
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $limit,
			'url'   => $this->url->link('product/winter', 'language=' . $this->config->get('config_language') . $pagination_url . '&page={page}')
		]);
		...
		$this->response->setOutput($this->load->view('product/winter', $data));
	}
