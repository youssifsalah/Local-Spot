	...
	public function index(): ?\Opencart\System\Engine\Action {
		...
		if ($product_info) {
			...
			if (isset($this->request->get['path'])) {
				$effective_path = (string)$this->request->get['path'];
				$path = '';

				$parts = explode('_', $effective_path);
				$base_category_id = (int)$parts[0];

				// If opened from a top-level category page (e.g. Svg), prefer the deepest assigned child category
				// so breadcrumbs become Svg > Tees > Product instead of Svg > Product.
				if (count($parts) === 1 && $base_category_id > 0) {
					$deepest_query = $this->db->query("SELECT cp.category_id
						FROM `" . DB_PREFIX . "category_path` cp
						INNER JOIN `" . DB_PREFIX . "product_to_category` p2c ON (p2c.category_id = cp.category_id)
						WHERE p2c.product_id = '" . (int)$product_id . "'
						  AND cp.path_id = '" . $base_category_id . "'
						  AND cp.category_id <> '" . $base_category_id . "'
						ORDER BY cp.level DESC
						LIMIT 1");

					if ($deepest_query->num_rows) {
						$deepest_category_id = (int)$deepest_query->row['category_id'];

						$path_query = $this->db->query("SELECT path_id
							FROM `" . DB_PREFIX . "category_path`
							WHERE category_id = '" . $deepest_category_id . "'
							ORDER BY level ASC");

						$path_ids = [];

						foreach ($path_query->rows as $row) {
							$path_ids[] = (int)$row['path_id'];
						}

						$base_index = array_search($base_category_id, $path_ids, true);

						if ($base_index !== false) {
							$effective_ids = array_slice($path_ids, $base_index);

							if ($effective_ids) {
								$effective_path = implode('_', $effective_ids);
								$parts = explode('_', $effective_path);
							}
						}
					}
				...
				if ($category_info) {
					...
					$data['breadcrumbs'][] = [
						'text' => $category_info['name'],
						'href' => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $effective_path . $url)
					];
				}
			}
			...
			if (isset($this->request->get['path'])) {
				$url .= '&path=' . ($effective_path ?? $this->request->get['path']);
			}
			...
			// prefer manufacturer name, fallback to local brand categories.
			$data['product_brand_name'] = $data['manufacturer'];
			$data['product_brand_href'] = $data['manufacturer'] ? $data['manufacturers'] : '';

			if (!$data['product_brand_name']) {
				$brand_category_map = [
					61  => 'Starnine',
					62  => 'Svg',
					493 => 'Seemly',
				...
				$brand_query = $this->db->query("SELECT DISTINCT cp.path_id AS category_id
					FROM `" . DB_PREFIX . "product_to_category` p2c
					INNER JOIN `" . DB_PREFIX . "category_path` cp ON (cp.category_id = p2c.category_id)
					WHERE p2c.product_id = '" . (int)$product_id . "'
					  AND cp.path_id IN (61,62,493,504,516,527,544)
					ORDER BY FIELD(cp.path_id, 61,62,493,504,516,527,544)
					LIMIT 1");

				if ($brand_query->num_rows) {
					$brand_category_id = (int)$brand_query->row['category_id'];
					$data['product_brand_name'] = $brand_category_map[$brand_category_id] ?? '';
					$data['product_brand_href'] = $this->url->link(
						'product/category',
						'language=' . $this->config->get('config_language') . '&path=492_' . $brand_category_id
					);
				}
			}
			...
			$stock_status_info = $this->model_localisation_stock_status->getStockStatus($stock_status_id);
				...
				// Stock badge state for product page styling.
				$data['is_in_stock'] = !$product_info['subtract'] || ((int)$product_info['quantity'] > 0);
			...
			$data['review'] = $this->load->controller('product/review');
				...
				if ($this->customer->isLogged()) {
					...
					$wishlist_products = $this->model_account_wishlist->getWishlist($this->customer->getId());
					$wishlist_product_ids = array_map('intval', array_column($wishlist_products, 'product_id'));

					$data['is_wishlist'] = in_array((int)$product_id, $wishlist_product_ids, true);
