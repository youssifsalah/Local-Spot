...
class Search extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		if (isset($this->request->get['brand_id'])) {
			$filter_brand_id = (int)$this->request->get['brand_id'];
		} else {
			$filter_brand_id = 0;
		}
		...
		if (isset($this->request->get['brand_id'])) {
			$url .= '&brand_id=' . (int)$this->request->get['brand_id'];
		}
		...
		// Local brand dropdown from Clothing children (Starnine/Svg/Seemly/Antikka/etc.)
		$this->load->model('catalog/category');
		$data['brand_filters'] = $this->model_catalog_category->getCategories(492);
		$data['brand_id'] = $filter_brand_id;
		$data['selected_brand_name'] = '';

		foreach ($data['brand_filters'] as $brand_filter) {
			if ((int)$brand_filter['category_id'] === $filter_brand_id) {
				$data['selected_brand_name'] = (string)$brand_filter['name'];
				break;
			}
		}

		$brand_clear_url = 'language=' . $this->config->get('config_language');

		if ($filter_search !== '') {
			$brand_clear_url .= '&search=' . urlencode(html_entity_decode($filter_search, ENT_QUOTES, 'UTF-8'));
		}

		if ($filter_description) {
			$brand_clear_url .= '&description=1';
		}

		if ($filter_tag !== '') {
			$brand_clear_url .= '&tag=' . urlencode(html_entity_decode($filter_tag, ENT_QUOTES, 'UTF-8'));
		}

		$data['brand_clear_href'] = $this->url->link('product/search', $brand_clear_url);
		$data['search_clear_href'] = $this->url->link('product/search', 'language=' . $this->config->get('config_language'));
		...
		if ($filter_search || $filter_tag) {
			$filter_data = [
				...
				'filter_category_id'  => $filter_brand_id,
				'filter_sub_category' => $filter_brand_id ? 1 : 0,
				'sort'                => $sort,
				'order'               => $order,
				'start'               => ($page - 1) * $limit,
				'limit'               => $limit
			...
			if (isset($this->request->get['search']) && $this->config->get('config_customer_search')) {
				...
				$search_data = [
					...
					'category_id'  => $filter_brand_id,
					'sub_category' => $filter_brand_id ? 1 : 0,
		...
		$data['category_id'] = $filter_brand_id;
		$data['sub_category'] = $filter_sub_category;
		$data['search_strip_text'] = $filter_search !== '' ? $filter_search : $data['selected_brand_name'];
