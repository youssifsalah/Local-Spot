...
class Category extends \Opencart\System\Engine\Controller {
	...
	public function index(): ?\Opencart\System\Engine\Action {
		...
		if (isset($this->request->get['subcats']) && $this->request->get['subcats'] !== '') {
			foreach (explode(',', (string)$this->request->get['subcats']) as $raw_id) {
				$cid = (int)$raw_id;
				if ($cid > 0) {
					$selected_subcategories[] = $cid;
				}
			}
			$selected_subcategories = array_values(array_unique($selected_subcategories));
		}
		...
		if ($category_info) {
			...
			if (isset($this->request->get['limit'])) {
				...
				$data['categories'] = [];
				$data['subcategory_menu'] = [];
				$data['subcategory_menu_ids'] = [];
			...
			$this->load->model('catalog/product');
				...
				// Subcategory dropdown source:
				// 1) show direct children when current category has children.
				// 2) if current category has no children, show siblings under its parent.
				$menu_results = $results;
				$menu_base_path = $path;

				if (!$menu_results && !empty($category_info['parent_id'])) {
					$menu_results = $this->model_catalog_category->getCategories((int)$category_info['parent_id']);

					$path_parts = explode('_', (string)$path);
					array_pop($path_parts);
					$menu_base_path = implode('_', $path_parts);

					if (!$menu_base_path) {
						$menu_base_path = (string)$category_info['parent_id'];
					}
				}

				foreach ($menu_results as $menu_item) {
					$menu_path = $menu_base_path ? ($menu_base_path . '_' . $menu_item['category_id']) : (string)$menu_item['category_id'];

					$data['subcategory_menu'][] = [
						'category_id' => (int)$menu_item['category_id'],
						'name'        => $menu_item['name'],
						'href'        => $this->url->link('product/category', 'language=' . $this->config->get('config_language') . '&path=' . $menu_path . $url)
					];
					$data['subcategory_menu_ids'][] = (int)$menu_item['category_id'];
				}

				if ($selected_subcategories) {
					$selected_subcategories = array_values(array_intersect($selected_subcategories, $data['subcategory_menu_ids']));
				}
				$data['selected_subcategory_ids'] = $selected_subcategories;
			...
			if (isset($this->request->get['page'])) {
				...
				if (isset($this->request->get['subcats'])) {
					$url .= '&subcats=' . $this->request->get['subcats'];
				}

			// Product
			$data['products'] = [];
			$data['product_subcategory_ids'] = [];
				...
				// Show products from Clothing subcategories by default.
				if ($category_id === 492 && !$selected_subcategories) {
					$filter_data['filter_sub_category'] = true;
				}

				// Brand pages (children of Clothing) should include nested category products.
				if ((int)$category_info['parent_id'] === 492) {
					$filter_data['filter_sub_category'] = true;
				}
				...
				if ($selected_subcategories) {
					...
					$all_results = $this->model_catalog_product->getProducts($filter_data);
					$matched_ids = [];

					if ($all_results) {
						$product_ids = [];
						foreach ($all_results as $all_result) {
							$product_ids[] = (int)$all_result['product_id'];
						}

						if ($product_ids) {
							$pid_list = implode(',', array_unique($product_ids));
							$sid_list = implode(',', $selected_subcategories);

							$matched_query = $this->db->query("SELECT DISTINCT p2c.product_id
								FROM `" . DB_PREFIX . "product_to_category` p2c
								INNER JOIN `" . DB_PREFIX . "category_path` cp ON (cp.category_id = p2c.category_id)
								WHERE p2c.product_id IN (" . $pid_list . ")
								  AND cp.path_id IN (" . $sid_list . ")");

							foreach ($matched_query->rows as $matched_row) {
								$matched_ids[(int)$matched_row['product_id']] = true;
							}
						}
					}
					...
					$filtered_results = [];
					foreach ($all_results as $all_result) {
						if (isset($matched_ids[(int)$all_result['product_id']])) {
							$filtered_results[] = $all_result;
						}
					}

					$product_total = count($filtered_results);
			...
			foreach ($results as $result) {
				...
				if ($data['subcategory_menu_ids']) {
					$id_list = implode(',', $data['subcategory_menu_ids']);

					$subcat_query = $this->db->query("SELECT DISTINCT cp.path_id AS category_id
						FROM `" . DB_PREFIX . "product_to_category` p2c
						INNER JOIN `" . DB_PREFIX . "category_path` cp ON (cp.category_id = p2c.category_id)
						WHERE p2c.product_id = '" . (int)$result['product_id'] . "'
						  AND cp.path_id IN (" . $id_list . ")");

					foreach ($subcat_query->rows as $subcat_row) {
						$subcat_ids[] = (int)$subcat_row['category_id'];
					}
				}

				$data['product_subcategory_ids'][] = implode(',', $subcat_ids);
			}
